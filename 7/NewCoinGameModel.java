...
public interface NewCoinGameModel {
  ...
  /**
   * Gets the (zero-based) positions of the coins, in order. The resulting array has length {@code
   * coinCount()} and its elements are strictly ascending. Modifying the resulting array must have
   * no effect on the game model.
   *
  ...
  /**
   * Gets the play order as an array. This array should not reflect the current position in the play
   * order, but rather be fixed during game play except when new players are added. Modifying the
   * resulting array must have no effect on the game model.
   *
  ...
  /**
   * Adds a new player to the game, returning the {@link CoinGamePlayer} object representing the new
   * player. The position of the player in the game order is specified by passing the player who
   * should precede the new player.
   *
   ...
   * @throws IllegalArgumentException if {@code predecessor} is not a player in this game's play
   *                                  order.
   */
