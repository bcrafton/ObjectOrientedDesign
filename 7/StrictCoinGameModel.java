...
public final class StrictCoinGameModel implements CoinGameModel {
  ...
  */
  ...
  protected boolean isOccupied(int coinPosition) {
    if (positionInRange(coinPosition)) {
  ...
  /**
   * Checks whether {@code newPosition} is in range.
   *
   * @param coinPosition the position that is being checked
   * @return true or false, whether or not the {@code coinPosition} in range
   */
  protected boolean positionInRange(int coinPosition) {
    return coinPosition < gameBoard.length && coinPosition >= 0;
  }
  ...
  /**
   * returns a string representing the order of play with "|" around whos turn it is.
   *
